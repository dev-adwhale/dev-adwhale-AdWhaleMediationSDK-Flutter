...
class RewardAd extends AdWithoutView {
  RewardAd({
    required this.appCode,
    required this.adRewardLoadCallback,
  });
  ...
  /// Callback to be invoked when the ad finishes loading.
  final RewardAdLoadCallback adRewardLoadCallback;

  /// Loads an [RewardAd] with the given [adUnitId] and [request].
  Future<void> load() async {
    debugPrint('RewardAd.load()');
    if (Platform.isAndroid) {
      await instanceManager.loadRewardAd(this);
    }
  }
